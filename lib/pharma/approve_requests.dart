...
class ApprovePharmacyRequests extends StatefulWidget {
  const ApprovePharmacyRequests({Key? key}) : super(key: key);

  @override
  State<ApprovePharmacyRequests> createState() => _ApprovePharmacyRequestsState();
}

class _ApprovePharmacyRequestsState extends State<ApprovePharmacyRequests> {
      ...
      appBar: AppBar(
        ...
        title: const Row(
          ...
          children: <Widget>[
            ...
            Expanded(
              child: Center(
                child: Text('Approve Requests', style: TextStyle(color: Colors.white)),
              ),
            ),
          ],
        ),
  ...
  void acceptRequest(TelemedicineRequestModel request) {
    // Update the status in Firestore to 'Approved'
    _firestore
        .collection("telemedicine_requests")
        .doc(request.orderId)
        .update({'status': 'Approved'})
        .then((value) {
      // Refresh the UI after updating the status
      fetchTelemedicineRequests();
      // You can add a success message or perform any additional actions
    }).catchError((error) {
  ...
  void rejectRequest(TelemedicineRequestModel request) async {
    final String? rejectionReason = await _showRejectionReasonDialog();

    // Update the status and reason in Firestore to 'Rejected' with the reason
    await _firestore
        .collection("telemedicine_requests")
        .doc(request.orderId)
        .update({'status': 'Rejected\nReason: $rejectionReason'})
        .then((value) {
      // Use addPostFrameCallback to refresh the UI after the frame has been painted
  ...
  // Function to show the rejection reason dialog
